...
public:
    ...
    // run step
    // main info that env need return, other info via get_info
    void get_observation(GroupHandle group, float **linear_buffers) override;
    // set action by group, each group call one time before step
    void set_action(GroupHandle group, const int *actions) override;
    // execute actions of all groups simultaneously after all groups set actions
    void step(int *done) override;
    // get reward by group
    void get_reward(GroupHandle group, float *buffer) override;

    // info getter, get any other info via this function
    ...
    // special run step
    void clear_dead();
    // set goal position for each agent in this group
    void set_goal(GroupHandle group, const char *method, const int *linear_buffer);

    // agent
    // type define a class of agents, multiple different groups can have the same type, n is the number of keys
    void register_agent_type(const char *name, int n, const char **keys, float *values);
    // create a new group whose type is agent_name, return the new grouphandle group
    void new_group(const char *agent_name, GroupHandle *group);
    // add agents to a group, or add walls to world
    void add_agents(GroupHandle group, int n, const char *method,
                    const int *pos_x, const int *pos_y, const int *pos_dir);

    // Yuanshan Lin add, set an agent indexed by idx at a random pos
    bool set_agent_rand_pos(GroupHandle group, int idx);

    // reward description, their difinitions can be found in RewardEngine.cc
    // define a role of an agent or agents belonged to group, no is No. of the role; index: -1 for any, -2 for all
    void define_agent_symbol(int no, int group, int index);
    // define an event which can be detected by world, where no is No. of the event, op is event type, inputs are the event's inputs
    void define_event_node(int no, int op, int *inputs, int n_inputs);
    // define reward rule, where on is EventNode
...
private:
    ...
    // Yuanshan Lin
    bool pbc_mode;       // Periodic boundary conditions (PBCs)
...
public:
    Agent(AgentType &type, int id, GroupHandle group) : dead(false), absorbed(false), group(group),
        ...
        orientation = 0;//rnd(e);    // 调试，暂时设为0.在gridword的add_agents中真正被初始化，之后在Map的do_move中进行不断更新
        real_pos_x = real_pos_y = 0.0;      // 在gridword的add_agents中真正被初始化，之后在Map的do_move中进行不断更新
...
//    const float &get_orientation() const { return orientation; }
//    void set_orientation(float orientation) { this->orientation = orientation; }
...
//    // Yuanshan Lin
//    void action2delta(int act, int &dx, int &dy, float &da) {
//        float interval = type.move_angle / type.move_n;  // 以角度为单位
//        da = (act - int(type.move_n/2)) * interval;
//        float angle = orientation + da;
//        dx = int(type.speed * cos(angle*PI/180.0) + 0.5);
//        dy = int(type.speed * sin(angle*PI/180.0) + 0.5);
//
////        std::cout << interval << "\t" << da << "\t" << angle << "\t" << dx << "\t" << dy << std::endl;
//    }
