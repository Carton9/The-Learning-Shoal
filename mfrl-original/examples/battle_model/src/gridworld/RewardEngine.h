...
class EventNode {
public:
    EventOp op;   // OP_AND, OP_OR, OP_NOT, OP_KILL, OP_AT, OP_IN, OP_COLLIDE, OP_COLLIDE_WALL, OP_ATTACK, OP_DIE, OP_IN_A_LINE, OP_ALIGN, OP_NULL
...
public:
    ...
    EventNode *on;        // triggered by which EventNode

    std::vector<AgentSymbol*> receivers;     // which agents can receive reward
    std::vector<float> values;               // how much reward can receivers get
    bool is_terminal;
    bool auto_value;
