...
//    std::cout << "GGGGGGGGGGGGGG " << std::endl;
    ...
    width = length = 1;
    speed = 1.0; move_angle = 360; move_n = 0;
    hp = 1.0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        AGENT_TYPE_SET_INT(action_type);
        AGENT_TYPE_SET_FLOAT(move_angle);
        AGENT_TYPE_SET_INT(move_n);
...
//        std::cout << "action_type: " << action_type << std::endl;
    ...
    if(action_type == 0){
        ...
        LOG(FATAL) << "INVALID Action Type in AgentType::AgentType.";
    }
//    std::cout << "turn base: " << turn_base << std::endl;
