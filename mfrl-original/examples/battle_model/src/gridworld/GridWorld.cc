...
void GridWorld::reset() {
    ...
    // reset map
    map.reset(width, height, food_mode, pbc_mode);
...
void GridWorld::set_config(const char *key, void *p_value) {
    ...
    if (strequ(key, "map_width"))
        ...
        minimap_mode = bvalue;
    else if (strequ(key, "pbc_mode"))       // set the world with Periodic boundary conditions (PBCs)
        pbc_mode = bvalue;
    ...
    if (group == -1) {  // group == -1 for wall
        if (strequ(method, "random")) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                std::cout << "wall at (" << pos_x[i] << ", " << pos_y[i] << ")" << std::endl;
                map.render();
            }
        ...
        std::uniform_real_distribution<double> rnd(-PI, PI);
...
//                std::cout << "dir: " << dir << std::endl;
                ...
                // Yuanshan Lin
                // 1. 根据dir初始化agent的orientation; enum {EAST, SOUTH, WEST, NORTH, DIR_NUM}
                switch(dir)
                {
                case EAST: agent->orientation = 0.0; break;
                case SOUTH: agent->orientation = -90.0; break;
                case WEST: agent->orientation = -179.99; break;
                case NORTH: agent->orientation = 90.0; break;
                default: LOG(FATAL) << "unsupported direction in GridWorld::add_agents : " << dir;
                }

                agent->orientation = rnd(e);
//                std::cout << "initial orientation: " << agent->orientation << std::endl;
                // 2. 初始化agent的实数类型位置值
                agent->real_pos_x = pos.x;
                agent->real_pos_y = pos.y;

//                std::cout << "real_pos: (" << pos.x << ", " << pos.y << ")\n";
            ...
            for (int i = 0; i < n; i++) {
                ...
                case WEST: agent->orientation = -179.99; break;
                case NORTH: agent->orientation = 90.0; break;
                default: LOG(FATAL) << "unsupported direction in GridWorld::add_agents : " << pos_dir[i];
                }

                agent->orientation = rnd(e);
//                std::cout << "orientation: " << agent->orientation << std::endl;

                // 2. 初始化agent的实数类型位置值
                agent->real_pos_x = pos_x[i];
                agent->real_pos_y = pos_y[i];

//                std::cout << "real_pos: (" << agent->real_pos_x << ", " << agent->real_pos_y << ")\n";
...
bool GridWorld::set_agent_rand_pos(GroupHandle group, int idx){
    Group &g = groups[group];
    std::vector<Agent*> &agents = g.get_agents();
    Agent* agent = agents[idx];

    bool success = map.set_agent_rand_pos(agent);
    return success;
}
...
void GridWorld::get_observation(GroupHandle group, float **linear_buffers) {
    ...
    for (int i = 0; i < agent_size; i++) {
        ...
        // Yuanshan Lin
        if(pbc_mode){
            map.extract_view_wrap(agent, view_buffer.data + i*view_height*view_width*n_channel, &channel_trans[0], range,
...
void GridWorld::set_action(GroupHandle group, const int *actions) {
    ...
    size_t agent_size = agents.size();
//    std::cout << "GridWorld::set_action: " << std::endl;
...
//        std::cout << "TTTTTTTTTTTTTT " << move_size << std::endl;
        for (int j = 0; j < move_size; j++) {
            ...
            // Yuanshan Lin
            Reward rew;
            bool success = map.do_move(agent, act, rew);
        }
    ...
    for (int i = 0; i < group_size; i++) {
        ...
        for (int j = 0; j < agent_size; j++) {
            ...
            if (agent->is_dead()) {
                delete agent;
                dead_ct++;
            } else {          // reset other agents' index because of dead agents deleted
                agent->init_reward();
                agent->set_index(pt);
                agents[pt++] = agent;
        ...
        agents.resize(pt);       // size of agents reduce to pt
        group.set_dead_ct(0);    // set 0, not dead_ct++ ?????!!!!
    }
}
...
void GridWorld::get_info(GroupHandle group, const char *name, void *void_buffer) {
    ...
    } else if (strequ(name, "real_pos")) {   // float
        size_t agent_size = agents.size();
        #pragma omp parallel for
        for (int i = 0; i < agent_size; i++) {
            float_buffer[2 * i] = agents[i]->real_pos_x;
            float_buffer[2 * i + 1] = agents[i]->real_pos_y;
        }
    } else if (strequ(name, "orientation")) {   // float
        size_t agent_size = agents.size();
        #pragma omp parallel for
        for (int i = 0; i < agent_size; i++) {
            float_buffer[i] = agents[i]->orientation;
        }
    }
    else if (strequ(name, "last_event_op")) {   // int
        size_t agent_size = agents.size();
        #pragma omp parallel for
        for (int i = 0; i < agent_size; i++) {
            int_buffer[i] = agents[i]->get_last_op();
        }
    } else {
...
std::vector<int> GridWorld::make_channel_trans(
    ...
    for (int i = 0; i < groups.size(); i++) {
        ...
        if (minimap_mode) {
            base += 6;         // has, hp, minimap, orientation, previous_act, id
        } else {
            base += 5;        // has, hp, orientation, previous_act, id
        }
    }
    return trans;
}
...
int GridWorld::group2channel(GroupHandle group) {
    int base = 1;
    int scale = 5;   // has, hp, orientation, previous_act, id
    if (food_mode)
        base++;
    if (minimap_mode)
        scale++;

    return base + group * scale; // wall + additional + (has, hp, orientation, previous_act, id) + (has, hp, orientation, previous_act, id) + ...
}
