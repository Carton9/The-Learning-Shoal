...
public:
    ...
    void reset(int width, int height, bool food_mode, bool pbc_mode);
    ...
    void extract_view_wrap(const Agent *agent, float *linear_buffer, const int *channel_trans, const Range *range,
                      int n_channel, int width, int height, int view_x_offset, int view_y_offset,
                      int view_left_top_x, int view_left_top_y,
                      int view_right_bottom_x, int view_right_bottom_y) const;
    ...
    // Yuanshan Lin
    bool do_move(Agent *agent, int act, Reward &r);
    Reward do_turn(Agent *agent, int wise);
    bool set_agent_rand_pos(Agent *agent);
...
//        std::cout << "In pos2int: " << x << ", " << y << std::endl;
