...
def get_config(map_size):
    ...
    cfg.set({"pbc_mode": True})         # Periodic boundary conditions (PBCs)
    # cfg.set({"agents_num": agents_num})
    cfg.set_view_space_spec(['pos', 'orientation'])
    ...
    fish = cfg.register_agent_type(
        ...
        {'width': 1, 'length': 1, 'hp': 5,
         'action_type': 1,               # 0: 原系统表达方式; 1: 线速度恒定，离散化角速度作为action； >2预留
         'speed': 3,
         'move_angle': 2*np.pi/3,        # 弧度为单位
         'move_n': 361,                  # 要求奇数
         'view_range': gw.CircleRange(4), #gw.SectorRange(5, 120),     # view_space=(视野范围占据矩形高, 视野范围占据矩形宽, channel)，其中channel= minimap_mode ? 7, 5； gw.CircleRange(5, 120)——view_space=(2*5+1, 2*5+1, channel)， gw.SectorRange(5, 120)——view_space=(5, 9, channel)
