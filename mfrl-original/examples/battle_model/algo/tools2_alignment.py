...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
        ...
        if self.train:
            ...
            summary_items = ['total_reward', 'ave_reward_agent_step', 'order_params', 'avg_neighbors_num']
    ...
    def run(self, variant_eps, iteration, win_cnt=None, results_dir='', n_agents=0):
        # pass
        info = {'total_reward': 0., 'ave_reward_agent_step': 0., 'order_params': 0.0, 'avg_neighbors_num': 0.}

        max_nums, nums, agent_r_records, total_rewards, agent_acc_rewards, order_params, avg_neighbors_num, agent_infos = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
        ...
        latter_part_order_params = order_params[0][int(self.max_steps * 0.3) : self.max_steps]
        latter_part_avg_neighbors_num = avg_neighbors_num[0][int(self.max_steps * 0.3): self.max_steps]
        # print(latter_part_avg_neighbors_num)
        # print(np.mean(latter_part_avg_neighbors_num))
        # print(sum(latter_part_avg_neighbors_num) / len(latter_part_avg_neighbors_num))
        # print(np.mean(latter_part_order_params))
        # print(sum(latter_part_order_params) / len(latter_part_order_params))
        # print(order, len(order))
        # print(sum(np.array(order_params[0])) / self.max_steps)
        # print(sum(latter_part_order_params) / len(latter_part_order_params))
        info['total_reward'] = total_rewards  # 本回合该小组的总的回报
        info['ave_reward_agent_step'] = agent_r_records  # 每个智能体拿到的平均回报
        info['order_params'] = np.mean(latter_part_order_params)
        info['avg_neighbors_num'] = np.mean(latter_part_avg_neighbors_num)
        # info['sum_Reward'] = agent_acc_rewards
        ...
        if self.train:
            ...
            file_name = '/order_{}_{}.txt'.format(n_agents, iteration)
            np.savetxt(results_dir + file_name, np.array(order_params), fmt="%.2f")
            file_name = '/neighbors_num_{}_{}.txt'.format(n_agents, iteration)
            np.savetxt(results_dir + file_name, np.array(avg_neighbors_num), fmt="%.2f")

            # print(agent_infos[0])
            # print(agent_infos[0].shape)
            file_name = '/agent_infos_{}_{}'.format(n_agents, iteration)
            np.save(results_dir + file_name, agent_infos[0])#.tofile(file_name, sep=',', format='%s')
        ...
        return info['order_params'], info['avg_neighbors_num']
