...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
        ...
        if self.train:
            ...
            summary_items = ['alive', 'ave_reward_agent_step', 'total_reward', 'kill', "Sum_Reward", "Kill_Sum"]
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        # pass
        info['main'] = {'alive': 0., 'kill': 0., 'total_reward': 0., 'ave_reward_agent_step': 0.}
        info['opponent'] = {'alive': 0., 'kill': 0., 'total_reward': 0., 'ave_reward_agent_step': 0.}

        max_nums, nums, agent_r_records, total_rewards, agent_acc_rewards = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
        ...
        for i, tag in enumerate(['main', 'opponent']):
            info[tag]['total_reward'] = total_rewards[i]     # 本回合该小组的总的回报
            info[tag]['alive'] = nums[i]  # 该小组存活agent的数量
            info[tag]['kill'] = max_nums[i] - nums[i]    # 该小组总共被杀死的数量
            info[tag]['ave_reward_agent_step'] = agent_r_records[i]   # 每个智能体拿到的平均回报
